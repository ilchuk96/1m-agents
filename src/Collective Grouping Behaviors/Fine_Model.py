...
class Fine_Model:
    def __init__(self, args):
        ...
        with tf.variable_scope('layer_output_fine'):
            self.W = tf.get_variable(name='weights',
    ...
    def infer_actions(self, sess, input_view, policy='e_greedy', epsilon=0.1):
        ret_actions = []
        ret_actions_batch = []
    ...
    def infer_max_action_values(self, sess, input_view):
        ret = []
        batch_id, batch_view = self.process_view_with_emb_batch(input_view)
    ...
    def process_view_with_emb_batch(self, input_view):
        return 0, np.concatenate((self.agent_embeddings, input_view), 0)
