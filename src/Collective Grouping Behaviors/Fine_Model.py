...
class Fine_Model_DNN():
    def __init__(self, args):
        ...
        with tf.variable_scope('layer_output'):
            self.W = tf.get_variable(name='weights',
                                     initializer=tf.truncated_normal([last_hidden_size, self.args.fine_num_actions],
                                                                     stddev=0.1))
            self.b = tf.get_variable(name='bias', initializer=tf.zeros([self.args.fine_num_actions]))
            self.output = tf.matmul(self.x, self.W) + self.b  # batch_size x output_size

        # Train operation
        self.reward_decay = self.args.reward_decay
        self.actions_onehot = tf.one_hot(self.actions, self.args.fine_num_actions)
    ...
    def _inference(self, sess, input_view, if_sample, policy='e_greedy', epsilon=0.1):
        ...
        if if_sample:
            if policy == 'greedy':
                ...
                return actions
            if policy == 'e_greedy':
                all_actions = range(self.args.fine_num_actions)
                actions = []
